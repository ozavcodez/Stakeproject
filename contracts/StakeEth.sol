...
contract StakeEth {
    ...
    function reward(address _staker) {
        ...
        uint reward = (stake.amount * REWARD_RATE * duration) /
            (SECONDS_IN_30_DAYS * 100);
        return reward;
    }

    function withdraw() public {
        EthStake memory stake = balance[msg.sender];
        require(stake.amount > 0, "No staked Ether found");

        uint reward = _calculateReward(msg.sender);
        uint totalAmount = stake.amount + reward;

        balance[msg.sender] = EthStake({amount: 0, stakeTime: 0}); // Reset user's stake
